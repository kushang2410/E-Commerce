...
function updateDataItems() {
    const cartItemsMobile = document.getElementById('cart-items-1-mobile');
    const cartItems = document.getElementById('cart-items-1');
    ...
    if (cart.length > 0) {
        ...
        if (cartItemsMobile) cartItemsMobile.innerHTML = tabDataItems;
        if (cartItems) cartItems.innerHTML = tabDataItems;
    } else {
        if (cartItemsMobile) cartItemsMobile.innerHTML = '<img src="assest/images/empty-cart.png" class="empty-cart" alt="">';
        if (cartItems) cartItems.innerHTML = '<img src="assest/images/empty-cart.png" class="empty-cart" alt="">';
    }
    const subTotal = cart.reduce((total, item) => total + (item.price * item.quantity), 0);
    const subTotalMobile = document.getElementById('sub-total-mobile');
    const subTotalElement = document.getElementById('sub-total');
    if (subTotalMobile) subTotalMobile.innerText = `$${subTotal.toFixed(2)}`;
